...
IMPORTANT: We do not publish Docker images so far. This deployment approach assumes that you have pre-built the images `skoop/webapp:latest` and `skoop/server:latest` on the Docker daemon which will run the containers!
...
. Start the deployment: `docker stack deploy --compose-file docker-compose.yml skoop`
